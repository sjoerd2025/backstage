...
export class ExtensionTester<UOutput extends ExtensionDataRef> {
  ...
  snapshot(): ExtensionSnapshotNode {
    ...
    // Validate that the tree structure is actually a tree (no multiple parents)
    const parentCounts = new Map<string, number>();
    for (const node of tree.nodes.values()) {
      if (node.edges.attachedTo) {
        const count = parentCounts.get(node.spec.id) ?? 0;
        parentCounts.set(node.spec.id, count + 1);
        if (count > 0) {
          throw new Error(
            `Extension '${node.spec.id}' has multiple parents, which is not supported in extension tree snapshots`,
          );
        }
      }
    }
